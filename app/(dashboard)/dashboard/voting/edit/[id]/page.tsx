...
// Convert ISO or datetime string → datetime-local input value
...
const EditCompetitionPage = () => {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      voteStart: toLocalInput(existing.voteStart),
      voteEnd: toLocalInput(existing.voteEnd),
      pricing: existing.pricing ?? "free",
      pricePerVote: existing.pricePerVote ?? 0,
      showLiveCount: existing.showLiveCount ?? true,
      publicLeaderboard: existing.publicLeaderboard ?? true,
      banner: existing.banner ?? null,
    });

    if (existing.banner) setBannerPreview(existing.banner);

    const mapped: Contestant[] = (existing.contestants ?? []).map(
      (c: any, i: number) => ({
        id: c.id ?? Date.now() + i,
        name: c.name ?? "",
        tagline: c.tagline ?? "",
    ...
    setContestants(mapped.length >= 2 ? mapped : [
      ...mapped,
      ...Array.from({ length: 2 - mapped.length }, (_, i) => ({
        id: Date.now() + i,
        name: "", tagline: "", category: "", photoUrl: null,
      })),
    ]);

    // Seed photo previews with existing URLs
    const previews: Record<number, string | null> = {};
    mapped.forEach((c) => { if (c.photoUrl) previews[c.id] = c.photoUrl; });
    setPhotoPreviews(previews);

    setHydrated(true);
  }, [existing, hydrated]);

  const anyUploading = bannerUploading || Object.values(photoUploading).some(Boolean);
  ...
  const clearContestantError = (id: number, field: keyof Omit<Contestant, "id">) =>
    setErrors((prev) => {
      ...
      const cs = { ...prev.contestants };
      const entry = { ...cs[id] };
      delete entry[field];
      cs[id] = entry;
      return { ...prev, contestants: cs };
    });

  // ─── Form handlers ───────────────────────────────────────────────────────────

  const updateForm = <K extends keyof CompetitionForm>(field: K, value: CompetitionForm[K]) => {
  ...
  const handleBannerFile = async (file: File) => {
    ...
    try {
      ...
      setErrors((prev) => ({ ...prev, banner: err.message ?? "Banner upload failed." }));
  ...
  const handlePhotoFile = async (contestantId: number, file: File) => {
    if (!file.type.startsWith("image/")) {
      setErrors((prev) => ({
        ...
        contestants: {
          ...prev.contestants,
          [contestantId]: { ...prev.contestants?.[contestantId], photoUrl: "Only image files are allowed." },
        },
      }));
      return;
    }
    ...
    try {
      ...
      setErrors((prev) => ({
        ...
        contestants: {
          ...prev.contestants,
          [contestantId]: { ...prev.contestants?.[contestantId], photoUrl: err.message ?? "Photo upload failed." },
        },
      }));
  ...
  const removePhoto = (contestantId: number) => {
    setPhotoPreviews((prev) => ({ ...prev, [contestantId]: null }));
    setContestants((prev) => prev.map((c) => (c.id === contestantId ? { ...c, photoUrl: null } : c)));
    clearContestantError(contestantId, "photoUrl");
    if (fileInputRefs.current[contestantId]) fileInputRefs.current[contestantId]!.value = "";
  };
  ...
  const validate = (): boolean => {
    ...
    if (form.voteStart && form.voteEnd && new Date(form.voteEnd) <= new Date(form.voteStart)) {
    ...
    if (Object.keys(contestantErrors).length) newErrors.contestants = contestantErrors;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await updateVote(id, payload);
  ...
  if (isLoading) {
    return (
      <div className="max-w-5xl mx-auto p-6 flex flex-col gap-4 animate-pulse">
        ...
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-white/5 rounded-xl h-12" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto flex flex-col gap-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-extrabold text-white">Edit Competition</h1>
          <p className="text-gray-500 text-xs mt-0.5">{existing.title}</p>
        </div>
      </div>
  ...
  <div className={radioCard(true)}>
    <div>
      <p className="text-white text-sm font-medium">
        {form.pricing === "free" ? "Free" : "Paid"}
      </p>
      <p className="text-gray-500 text-xs mt-0.5">
        {form.pricing === "free"
          ? "Anyone can vote at no cost"
          : `₦${form.pricePerVote.toLocaleString()} per vote`}
      </p>
    </div>
    <span className="ml-auto text-[10px] text-gray-600 bg-white/5 border border-white/10 px-2 py-1 rounded-full">
      Cannot be changed
    </span>
  </div>
</section>
              ...
              return (
                <div
                  ...
                  className={`bg-[#0f0f0f] border rounded-2xl overflow-hidden flex flex-col transition ${
                    Object.keys(ce).length ? "border-red-500/40" : "border-[#1f1f1f]"
                  }`}
                >
                  {/* PHOTO */}
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <label className={radioCard(form.showLiveCount === true)}>
                <input type="radio" name="showLiveCount" checked={form.showLiveCount === true} onChange={() => updateForm("showLiveCount", true)} className="accent-[#FFD159]" />
                <div>
              ...
              <label className={radioCard(form.showLiveCount === false)}>
                <input type="radio" name="showLiveCount" checked={form.showLiveCount === false} onChange={() => updateForm("showLiveCount", false)} className="accent-[#FFD159]" />
                <div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <label className={radioCard(form.publicLeaderboard === true)}>
                <input type="radio" name="publicLeaderboard" checked={form.publicLeaderboard === true} onChange={() => updateForm("publicLeaderboard", true)} className="accent-[#FFD159]" />
                <div>
              ...
              <label className={radioCard(form.publicLeaderboard === false)}>
                <input type="radio" name="publicLeaderboard" checked={form.publicLeaderboard === false} onChange={() => updateForm("publicLeaderboard", false)} className="accent-[#FFD159]" />
                <div>
      ...
      <style jsx>{`
        ...
        .input-base:focus { border-color: #FFD159; box-shadow: 0 0 0 1px #FFD159; }
        .input-base::placeholder { color: #555; }
        .input-error { border-color: #f87171 !important; box-shadow: 0 0 0 1px #f87171 !important; }
        input[type="datetime-local"].input-base::-webkit-calendar-picker-indicator { filter: invert(0.5); }
...
const input = (hasError: boolean) => `input-base${hasError ? " input-error" : ""}`;
const contInput = (hasError: boolean) => `cont-input-base${hasError ? " cont-input-error" : ""}`;
