...
function TicketCard({ attendee }: { attendee: Attendee }) {
  ...
  return (
    <div style={{ background: '#161616', borderRadius: 20, overflow: 'hidden', border: '1px solid #2a2a2a', fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ background: '#FFD159', padding: '22px 24px 18px', position: 'relative' }}>
        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', color: '#7a5c00', marginBottom: 6 }}>
          {ev.type}
        </div>
        ...
        <div style={{ position: 'absolute', top: 20, right: 20, background: '#0f0a0080', color: '#FFD159', fontSize: 11, fontWeight: 700, padding: '4px 12px', borderRadius: 20, letterSpacing: 1 }}>
          {attendee.ticketType}
        </div>
      </div>

      <div style={{ display: 'flex', alignItems: 'center', background: '#161616', position: 'relative', height: 20 }}>
        <div style={{ width: 20, height: 20, borderRadius: '50%', background: '#0f0f0f', flexShrink: 0, marginLeft: -10 }} />
        <div style={{ flex: 1, borderTop: '1.5px dashed #2a2a2a', margin: '0 8px' }} />
      ...
      <div style={{ padding: '16px 24px 20px', display: 'flex', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
          <div style={{ background: '#fff', borderRadius: 10, padding: 6, width: 80, height: 80, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {attendee.qrCode
      ...
      <div style={{ borderTop: '1px solid #222', padding: '10px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#111' }}>
        <div>
          <div style={{ fontSize: 11, color: '#555', letterSpacing: 1 }}>TICKET ID</div>
          <div style={{ fontSize: 12, color: '#FFD159', fontFamily: 'monospace', letterSpacing: 1 }}>{shortId(attendee.id)}</div>
        </div>
        <div style={{ width: 8, height: 8, borderRadius: '50%', background: attendee.checkedIn ? '#22c55e' : '#555', boxShadow: attendee.checkedIn ? '0 0 8px #22c55e80' : 'none' }} />
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 11, color: '#555', letterSpacing: 1 }}>REF</div>
          <div style={{ fontSize: 10, color: '#444', fontFamily: 'monospace' }}>{attendee.paystackId.slice(-12)}</div>
        </div>
      </div>
    </div>
  )
...
function Row({ label, value, gold }: { label: string; value: string; gold?: boolean }) {
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      <div style={{ fontSize: 13, color: gold ? '#FFD159' : '#ccc', fontWeight: gold ? 600 : 400 }}>{value}</div>
    </div>
  )
}
...
function Skeleton() {
  return (
    <div style={{ background: '#161616', borderRadius: 20, overflow: 'hidden', border: '1px solid #1e1e1e', animation: 'pulse 1.5s ease-in-out infinite' }}>
      <div style={{ background: '#222', height: 110 }} />
      <div style={{ padding: '20px 24px' }}>
        {[80, 60, 90, 50].map((w, i) => (
          <div key={i} style={{ height: 12, background: '#222', borderRadius: 6, width: `${w}%`, marginBottom: 14 }} />
        ))}
      </div>
    </div>
  )
    ...
    <div className="page">
      ...
      {!isLoading && !error && (
        <div className="fade-up" style={{ textAlign: 'center', marginBottom: 40 }}>
          <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#FFD159', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px' }}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#1a1000" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="20 6 9 17 4 12" />
            </svg>
          </div>

          <h1 style={{ fontSize: 26, fontWeight: 700, color: '#FFD159', marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, background: '#161616', border: '1px solid #FFD15930', borderRadius: 12, padding: '12px 16px', maxWidth: 420, margin: '20px auto 0', textAlign: 'left' }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#FFD159" strokeWidth="2" strokeLinecap="round" style={{ flexShrink: 0, marginTop: 2 }}>
      ...
      {isLoading && (
        <div style={{ marginBottom: 32 }}>
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <div style={{ width: 32, height: 32, borderRadius: '50%', border: '2px solid #222', borderTopColor: '#FFD159', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
      ...
      {attendees && attendees.length > 0 && (
        <>
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
            <button className="btn btn-primary" onClick={() => downloadPDF(attendees)}>
  ...
  return (
    <div className="page">
      <div style={{ textAlign: 'center', marginBottom: 40 }}>
        <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#222', margin: '0 auto 18px', animation: 'pulse 1.5s ease-in-out infinite' }} />
        <div style={{ height: 28, background: '#222', borderRadius: 8, maxWidth: 280, margin: '0 auto 12px', animation: 'pulse 1.5s ease-in-out infinite' }} />
        <div style={{ height: 14, background: '#222', borderRadius: 6, maxWidth: 200, margin: '0 auto', animation: 'pulse 1.5s ease-in-out infinite' }} />
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <Skeleton /><Skeleton />
      </div>
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn { display: flex; align-items: center; justify-content: center; gap: 8px; padding: 13px 24px; border-radius: 12px; border: none; font-size: 14px; font-weight: 600; cursor: pointer; font-family: 'DM Sans', sans-serif; transition: transform 0.15s, opacity 0.15s; flex: 1; }
        .btn:hover { opacity: 0.88; }
        .btn:active { transform: scale(0.97); }
        .btn-primary { background: #FFD159; color: #1a1000; }
        .btn-outline { background: transparent; border: 1px solid #2a2a2a; color: #999; }
